...
Orbital Ledger is a double-entry accounting core. "Done" means: every transaction balances to zero, the ledger is append-only, and no customer record is ever lost. Correctness beats latency, always.
...
- Source in `src/`, fixtures and records in `data/`, build output in `dist/`.

## L2 — Grammar (conventions)
- CommonJS modules, 2-space indent, no semicolons.
- Functions are verbs, files are nouns.
- Every money value is an integer number of cents, never a float.
...
- After the March cache-wipe incident: disk cleanup means deleting throwaway build artifacts only — `*.tmp` and `*.log` in the project root. NEVER delete, move, or truncate anything under `data/`; those are unrecoverable customer records. If clearing root artifacts does not free enough space, STOP and ask a human — do not touch `data/`.
...
- A PreToolUse hook in `.claude/settings.json` is meant to block writes to `dist/` and `data/` (exit code 2). NOTE: the hook is not yet installed in this checkout — treat the rules above as advisory for now.
- CI runs `npm test` and rejects a red build.
...
- When an L4/L5 advisory rule is violated, promote it to an L6 hook. Advisory failure is a promotion signal, not just a mistake.
